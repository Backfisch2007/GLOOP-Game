...
public class BowlingSpiel {
    ...
    private Ball ball;
    private GLQuader[] kollisionsQuader;
    private BowlingPin[] pins;
    private Scoreboard scoreboard;
    private int aktuellerSpieler;
    private int versucheSpieler1;
    private int versucheSpieler2;

    private double quaderBreite = 5;
    private double quaderHoehe = 25;
    private double quaderTiefe = 5;

    public BowlingSpiel() {
        himmel = new GLHimmel("rosendal_plains_2.jpg");
        input = new GLTastatur();

        licht = new GLLicht(0, 100, -325); // Licht 1
        licht2 = new GLLicht(0, 100, 125); // Licht 2 weiter hinten um alles gut auszuleuchten
        licht.setzeAbschwaechung(0.5); // Etwas dunkler
        licht2.setzeAbschwaechung(0.5); // Etwas dunkler

        ball = new Ball(0, -200); // Position Ball

        steuerung = new KameraSteuerung();
        steuerung.setzePosition(0, 100, 10); // Position Kamera
        steuerung.setzeBlickpunkt(0, 5, 175); // Blickpunkt Kamera auf Pins

        bahn = new GLQuader(0, -5, 0, 50, 5, 400); // Bahn
        bahn.setzeFarbe(255, 255, 255); // Weiß damit Textur hell genug 
        bahn.setzeTextur("wood_floor_diff_4k.jpg"); // Holzboden

        scoreboard = new Scoreboard();

        kollisionsQuader = new GLQuader[10]; 
        double[] pinXPositionen = { -6.4, 6.4, -19.2, 19.2, 0, -12.8, 12.8, -6.4, 6.4, 0 };
        double[] pinZPositionen = { 185, 185, 185, 185, 175, 175, 175, 165, 165, 155 };
        ...
        for (int i = 0; i < 10; i++) {
            ...
            kollisionsQuader[i].setzeSichtbarkeit(false);
        }
        ...
        while (!input.esc()) {
            ...
            if (input.istGedrueckt('u')) {
                ball.setzeWinkel(ball.gibWinkel() + 2); // Winkel nach links erhöhen
                System.out.println("Winkel: " + ball.gibWinkel());
            }
            if (input.istGedrueckt('o')) {
                ball.setzeWinkel(ball.gibWinkel() - 2); // Winkel nach rechts verringern
                System.out.println("Winkel: " + ball.gibWinkel());
            }

            // Bewegung der Kugel vor dem Stoß (Positionierung)
            if (input.istGedrueckt('j')) {
                ball.bewege(2.5, 0); // Links (an Winkel angepasst)
            }
            if (input.istGedrueckt('l')) {
                ball.bewege(-2.5, 0); // Rechts (an Winkel angepasst)
            }
            ...
            if (input.enter()) {
                ...
                while (ball.gibZ() < 200) { // Solange der Ball auf der Bahn ist
                    ball.bewege(0, 8); // Bewegt den Ball in die Richtung des eingestellten Winkels
                    Sys.warte(40);
                ...
                ball.setzeWinkel(0); // Winkel zurücksetzen
    ...
    private boolean kollisionErkannt(BowlingPin pin1, BowlingPin pin2) {
        ...
        // Kollision, wenn der Abstand kleiner als ein bestimmter Wert ist (z.B. 5 Einheiten)
        return entfernung < 5;
    }
    ...
    private void reset() {
        ...
        if (input.istGedrueckt('r')) {
            ...
            //scoreboard.zuruecksetzenPunkte(); // Scoreboard zurücksetzen (falls gewünscht)
            System.out.println("Alle Pins, Ball, Kamera und Scoreboard wurden zurückgesetzt.");
            Sys.warte(1000); // Kurze Pause
        }
    }
}